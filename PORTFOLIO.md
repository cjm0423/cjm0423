...
- 파이프라인 **6단계 구간별 ms 단위 지연시간 추적 시스템** 직접 설계
- Baseline 포함 **5개 튜닝 시나리오** 설계·실험 (5회 반복, Welch's t-test로 통계적 유의성 검증)

**핵심 발견 (Table 9 기준, P95 지연시간)**

| 시나리오 | P95 지연 | Peak 처리량 | 패킷/초 | 안정성 |
| :--- | :--- | :--- | :--- | :--- |
| Baseline | 31.28초 | 449 req/s | 8,303 | 안정 (지연 과다) |
| Zero-Wait | 30.18초 | 309 req/s | 9,266 | 불안정 (처리량 스로틀링) |
| 마이크로배치 | **113.97초** (붕괴) | 2,267 req/s | 7,524 | 불안정 (스파이크) |
| I/O 제어 | 30.77초 | 298 req/s | 8,112 | 안정 |
| **하이브리드** | **28.45초** (최저) | 303 req/s | 9,161 | 안정 / 균형 |

- CPU 사용률(Baseline 전체 64.0%)은 여유 있었으나 병목은 리소스가 아니라 **버퍼링 정책**이었음 (Filebeat 대기가 전체 지연의 약 60% 차지)
- Zero-Wait는 지연 대신 패킷/초당 요청 수가 폭증하며 처리량이 오히려 300건/초 수준으로 제한됨
- 마이크로배치는 순간 처리량은 급증(최대 2,267 req/s)하지만 큐 백로그로 P95 지연이 113.97초까지 붕괴
- **하이브리드(전위 I/O 제어 + 후위 무대기 전송)가 전체 시나리오 중 가장 낮은 P95(28.45초, Baseline 대비 약 9% 개선, p=0.0402)를 기록하면서 마이크로배치의 붕괴를 방지**하고 300건/초 이상의 처리량을 유지

> 앞단 최적화가 뒷단 처리 용량을 넘어서면 병목은 사라지지 않고 **전이**된다.
> 튜닝은 절대적 수치 극대화가 아니라, 지연·처리량·안정성 간 **상충관계의 균형**을 찾는 문제다.
